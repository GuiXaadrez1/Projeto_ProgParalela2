...
def juntar_blocos_em_imagem_final(lista_blocos, caminho_saida_final):
    ...
    for _, caminho_img in lista_blocos:
        ...
        # Libera memória da imagem imediatamente após salvar em lista (caso deseje manter controle mais fino)
        # Não é obrigatório aqui, mas pode ajudar se houver MUITOS blocos
        gc.collect()

    imagem_final = np.vstack(imagens)
    cv2.imwrite(caminho_saida_final, imagem_final)

    # Limpa tudo o que pode estar ocupando memória
    del imagens, imagem_final
    gc.collect()
